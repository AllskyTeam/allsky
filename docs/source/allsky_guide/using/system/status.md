...
The IP address display is also important. If you connect to the WebUI by hostname most of the time, you may not think about the actual address often. But when remote access fails, when DHCP changes, or when you are configuring port forwarding or firewall rules, having the current IP addresses visible saves time.
...
!!! note
	Unlike the Windows operating system Linux rarely needs to reboot so unless you rebooted it, the host uptime should normally be a large number.

## The Allsky Information Panel { data-toc-label="Allsky Info" }

Alongside the host details, the **System** tab also shows information specifically about the Allsky software. This section usually includes:

- the installed Allsky version,
- whether the local Allsky Website is enabled,
- whether a remote Website is enabled,
- the Allsky uptime.
...
The installed version is useful when you are confirming an upgrade, comparing notes with the documentation, or preparing a support request (this information is also at the top left of every WebUI page).

The website status lines tell you whether the local and remote Allsky Websites are enabled. That matters because many users first notice a problem when images stop appearing on a Website. If the relevant Website is disabled, the explanation may be very simple. If it is enabled but the site still is not updating, you know to keep investigating.
...
Note that any negative conditions can only be cleared by rebooting the Pi.
...
Allsky temporary storage is another value that can become important during normal Allsky operation. Temporary Allsky files are often created as part of processing, uploads, and web-related tasks. If this space fills up, you may see failures that do not immediately look like disk-space problems unless you know where to look.  For that reason, it is worth treating temporary storage as a separate health signal rather than assuming it will always track the main disk usage in an obvious way.
...
See the [System Page Additions](../../howtos/system_page.md) page and the [System Page Additions Editor](status.md#additionsEditor) section below for more information.  An example of the System tab with additions is shown below.
...
- use the **Logs** tab when you need detailed evidence of what happened,
...
- the button label, for example: "Heater ON",
...
When you add a new entry or edit an existing one, the editor opens a form with fields appropriate to that entry type. You are shown the fields that make sense for the kind of item you are creating. <!-- rather than one long confusing form covering every possible case. -->
