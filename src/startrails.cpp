...
void startrail_worker(int thread_num,
	...
	int nchan = 3; // first maybe overexposed images (mono !) making problems
	...
	for (int f = start_num; f <= end_num; f++) {
		...
		if (cf->verbose > 1) {
			stdio_mutex.lock();
			fprintf(stderr, "[%d/%lu] %s %.3f\n", f + 1, nfiles, filename, image_mean);
			stdio_mutex.unlock();
		}
...
void usage_and_exit(int x) {
	std::cout << "Usage: startrails [-v] -d <dir> -e <ext> [-b <brightness> -o "
		"<output> | -S] [-s <WxH>] [-Q <max-threads>] [-q <nice>]" << std::endl;
	if (x) {
		std::cout << KRED
	...
	std::cout << "-Q | --max-threads <int> : limit maximum number of processing threads. (unspecified = use all cpus)" << std::endl;
	...
	std::cout << "-b | --brightness-limit <float> : ranges from 0 (black) to 1 (white). Default 0.35" << std::endl;
	std::cout << "\tA moonless sky may be as low as 0.05 while full moon can be as high as 0.4" << std::endl;

	std::cout << std::endl;
	std::cout << "ex: startrails -b 0.07 -d ../images/20180208/ -e jpg -o startrails.jpg" << std::endl;
	exit(x);
}
...
int main(int argc, char* argv[]) {
	...
	if (config.startrails_enabled) {
		if (accumulated.empty()) {
			fprintf(stderr, "No images below threshold %.3f, writing the minimum image only\n",
