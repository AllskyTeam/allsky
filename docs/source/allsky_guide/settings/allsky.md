...
???+ info "Settings"
    ...
    | ==Use Dark Frames== | No | Enable to perform dark frame subtraction at night. Requires that you first took dark frames using ==Take Dark Frames==.  <br><br>Hot pixels are replaced by the average of the pixels around them, so they don't turn into black dots. See [Hot pixels](/docs/allsky_guide/howtos/dark_frames.html#hot-pixels). |
