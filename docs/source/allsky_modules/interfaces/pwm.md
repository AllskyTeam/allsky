...
## Hardware PWM vs Software PWM { data-toc-label="Hardware PWM vs Software" }
...
## Raspberry Pi 4 – Hardware PWM Setup { data-toc-label="Raspberry Pi 4 –" }

The Raspberry Pi 4 uses the **BCM2711 SoC**, which provides two hardware PWM channels via the `bcm2835-pwm` driver.

### PWM-capable GPIOs (Pi 4) { data-toc-label="PWM-capable GPIOs (Pi 4)" }
...
### Enable PWM (Pi 4) { data-toc-label="Enable PWM (Pi 4)" }
...
## Raspberry Pi 5 – Hardware PWM Setup { data-toc-label="Raspberry Pi 5 –" }

Raspberry Pi 5 introduces a new I/O controller (**RP1**), so PWM is provided by a different driver.

### Enable PWM (Pi 5) { data-toc-label="Enable PWM (Pi 5)" }
...
## Hardware Warning { data-toc-label="Hardware Warning" }
