This page lists how to troubleshoot other problems that don't deserve their own page.
...
```
Sep 15 17:44:28 Pi-Allsky kernel: [108641.695297] usb 1-1.5:
...
png files can take 10 or more seconds to save on a Pi 4 and earlier because there is no hardware support for them. You may also see this message if the `Delay` between images is too short, e.g., less than a second.
...
- Increase your `Delay` setting in the WebUI's **Settings -> Allsky Settings** page.
- Save to `jpg` format instead of `png`.
- If you have an extremely slow SD card try replacing it with a faster one.
...
If the stars are too hard to see but the overall image image brightness is where you want it, try stretching the image.  See the [Helper Tools -> Stretch Settings](/index.php?page=stretch_settings) WebUI page for how to find the right values to use.

## Images are corrupted

Various forms of image corruption have been observed: the right side of the image is full of repeating pixels, an image contains parts of two or more frames, offset lines, etc.

- Check your power supply and cooling. The Pi will report on undervoltage and/or throttled status on the **Settings -> Allsky Settings** page of the WebUI. If it doesn't say `No throttling` you should investigate and remediate.
...
The git command is used to download Allsky releases from the GitHub server. The command rarely fails, but it can if there are networking problems or the GitHub server is down.

The first thing to check is your networking - can you get to a site on the Internet? If you can and you see unusual messages like below, the GitHub server is probably having problems so try again a few hours later.
