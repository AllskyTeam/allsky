...
!!! warning  "Warning"
    ...
    - Keograms only show what happens at the **center** of the image and will not display events on the sides.
...
The easiest way to manually create and optionally upload a keogram is via the `generateForDay.sh` command. For example, to create and then upload a keogram to any Allsky Website and/or remote server you have for July 10, 2026:

```
generateForDay.sh --keogram 20260710
generateForDay.sh --upload --keogram 20260710
```
...
```
generateForDay.sh --keogram --keogram-params "--font-color #ff0000" 20260710
```

This will create a keogram with a red timeline for July 10, 2026, overwriting any keogram that may already exist.
...
??? "Advanced users: running the keogram program manually"
    ...
    ```
    cd ~/allsky/bin
    ./keogram -d images/20260710 -e jpg -o images/20260710/keogram/keogram.jpg --rotate 42 --font-size 2
    ```
