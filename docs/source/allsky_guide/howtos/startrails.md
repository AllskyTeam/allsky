A startrail image shows the apparent paths of stars across the sky over an extended period of time. It is created by combining many individual images taken sequentially, typically over several hours, and stacking them so that each star’s movement forms a continuous arc.
...
!!! warning  "Warning"
    ...
    Only images that are saved are included in a startrails, so if you don't save your nightime images, they won't be included.
...
Make sure the extra parameters are enclosed in quotes.
...
## Create test startrails

See the [Startrails Helper Tool](helper_tools/startrails.md) documentation page to help set the `Brightness Threshold`.
<!--
