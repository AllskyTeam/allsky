Modules can be used to enhance images created by Allsky. Different modules perform different tasks and the **Module Manager** WebUI page allows you to control which modules run and when.

There are two types of modules:

  1. Allsky Modules are developed and maintained by the Allsky team and are installed with Allsky.

  2. User Modules are developed and maintained by other people and can optionally be installed and deleted if no longer required.

!!! warning  "Warning"

    User Modules should only be installed from the official Allsky [allsky-modules](https://github.com/AllskyTeam/allsky-modules) repository using the directions listed there. Installing from other source can be dangerous.
...
Modules are Python components that extend the functionality of Allsky. They typically fall into one of several categories:
  ...
  - **Data-enhancement modules** — These add extra information that can be used in overlays. For example, the Solar System module provides details about celestial objects like the Moon’s position.
  ...
  - **Image-processing modules** — These operate on the captured images. Some modify the image itself (like the Mask module), while others analyse the image to generate additional overlay data, like the Star Count module.
...
!!! info  "Default Pipline"
    ...
    When entering the Module Manager the default pipeline will be either Daytime Capture or Nighttime Capture depending upon the time of day.
...
|--------|----------|-------------|
| 1 | Save The Module Configuration | Use this button to save the pipeline after you have made any changes to modules or module configuration. |
| 2 | Select Pipeline| Selects the pipeline to manage. |
| 3 | Periodic Flow | Shortcut to display the periodic flow. |
| 4 | Module Filter | Filters modules in the available list by module type. |
| 5 | Package manager | Manage modules, install, remove. |
| 6 | Device Manager | Displays the 'Device Manager'. |
| 7 | Module Options | Displays the Module Manager settings. |
...
To enable a module drag it from the Available Modules column to the Selected Modules column and select the "Enabled" checkbox. The module will become active after the flow has been saved using the "Save The Module Configuration" button, which turns green after a change has been made. You can set modules to be automatically enabled when dragging them to the Selected Modules column via the Module Options button.

Most modules require some configuration after they have been moved to the Selected Modules column. Clicking a module's cog button will display any configuration options for the module.

### Disabling a Module { data-toc-label="Disabling a Module" }
There are two ways to disable a module:

  1. Uncheck the "Enabled" checkbox - this will disable the module but retain any settings. This is the preferred method if you just wish to temporarily disable a module.
  2. Drag the module to the Available Modules column - this will disable the module and lose any settings for it.

### Execution order { data-toc-label="Execution order" }
The modules are run in the order they appear in the Selected Modules column. To change the order simply drag the modules up or down the list. Locked modules can not be moved.

!!! info  "Module Order"

    The order modules run in is important. For example don't put the overlay module before a module that generates data for it. If you do, the overlay will display the data from the previous run of the pipeline.

## Module Settings { data-toc-label="Module Settings" }
Each module provides a dialog where its settings can be managed. These settings are divided into tabs. Each module will have different tabs but there are a few that are consistent across all modules.  Pressing a module's cog icon displays its settings.
...
This tab shows what variables the module will generate, these can then be used in the Overlay Editor's "Variable Manager" to add fields to the overlays. This is really only for information purposes.
...
This tab shows a history of the changes that have been made to the module.
