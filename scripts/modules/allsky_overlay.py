...
class ALLSKYOVERLAY:
	...
	def _processText(self, name, fieldData, pilImage = None):
		...
		for matchNum, match in enumerate(matches, start=1):
			...
			if 'type' in fieldData and fieldData['type'] != '':
				variableType = fieldData['type']
			else:
	...
	def _getValue(self, placeHolder, variableType, Format=None, empty=''):
		...
		if valueOk:
			...
			if fieldFound:
				if isinstance(variableType, str):
					variable_group = variableType
				else:
					variable_group = variableType['type']
				format_function = 'as_' + variable_group.lower()
				if hasattr(allskyformatters.allsky_formatters, format_function):
					self._log(4, f'Formatter {format_function} found. Value = {value}, format = {Format}')        
					try:
						if envCheck == 'DATE' or envCheck == 'AS_DATE' or envCheck == 'AS_TIME' or envCheck == 'TIME':
							value = self._imageDate
						value = getattr(allskyformatters.allsky_formatters, format_function)(value, envCheck, Format, variableType) 
					except AllskyFormatError as e:
						s.log(e.log_level, e.message, e.send_to_allsky)
					except Exception as e:
						pass
				else:
				...
				if 1 == 2:
					...
					elif variable_group == 'Bool':
						if Format is None or Format == '':
							Format = "%yes"
						value = self._doBoolFormat(rawFieldName, value, Format)

				if variable_group == 'Text' or variable_group == 'Number':
					if value == '' or value is None:
						if empty != '' and empty is not None:
							value = empty
				elif variable_group is None or variable_group == '':
				...
				elif variable_group == self._errorType:
