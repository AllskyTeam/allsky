...
??? warning "The Sun Moon and Planets"
...
!!! danger "This is a breaking change"
    ...
    The Dew Heater module has been re written so will need to be restup from scratch. The new module provides much easier ways to setup the dew heater. If you do not resetup the de heater then it will nto work.


If you previously used the dew heater module then you **MUST** now add the environment module, allsky_temp and setup the core temperature sensor. After installing the module select the 'Core' tab and configure the temperature sensor. You can use the test button to ensure the module is recording temperatures.

Once the temperature module is working you will need to reconfigure the Dew Heater module. The first step is to set the Sensor the dew heater will use. On the 'Sensor' tab select 'Allsky' as the sensor, this will then use the data captured by the environment module.

The dew heater functionality has been rewritten for this release so please refer to the [Documentation](/allsky_modules/extra/dew_heater.md) for details on how to setup the module 
