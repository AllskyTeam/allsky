...
??? warning "Dew heater module"

    If you previously used the dew heater module then you MUSt now add the environment module, allsky_temp and setup the core temperature sensor. After installing the module select the 'Core' tabe and configure the temperature sensor. You can use the test button to ensure the module is recording temperatures.

    Once the temperature module is working you will need to reconfigure the Dew Heater module. The first step is to set the Sensor the dew heater will use. On the 'Sensor' tab select 'Allsky' as the senor, this will then use the dat captured by the environment module.

    The dew heater functionality has been rewritten for this release so please refer to the [Documentation](/allsky_modules/extra/dew_heater.md) for details on howto setup the module 
