...
check_for_oldest()
{
	...
	if [[ -d ${OLDEST_DIR} ]]; then
		...
		local MSG2="\n\nIf you want to upgrade to the newest Allsky release, either remove '${OLDEST_DIR}'"
...
restore_directories()
{
	...
	mv "${PRIOR_ALLSKY_DIR}" "${ALLSKY_HOME}"
	if [[ -d ${OLDEST_DIR} ]]; then
...
function usage_and_exit()
{
	...
	if [[ ${RET} -eq 0 ]]; then
		echo "Upgrade the Allsky software to the newest version."
		echo -e "\n${USAGE}"
	else
		E_ "${USAGE}"
	fi
